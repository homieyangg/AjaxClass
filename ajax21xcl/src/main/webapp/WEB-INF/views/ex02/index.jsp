...
function showReq03Result(employees){
	...
	for(var n=0 ; n < array.length ; n++){
		var emp = array[n];
		var url_reqParam = "<c:url value='/ex04/req04/geImage' />?noo=" + emp.id;
		var url_pathVar = "<c:url value='/ex04/req04/geImage2' />/" + emp.id;
		console.log(url_pathVar);
...
function showReq04Result(mapData){
	...
	for(var n=0; n < obj.lotteryNumber.length; n++){
		console.log("元素" + (n+1) + ":" + obj.lotteryNumber[n]);
	}
...
<div align='center'>
   ...
   <button id='btn23'>傳回多個型態相同的物件: List<Employee></button>
   <button id='btn24'>傳回多個型態不同的物件: Map<String, Object></button>
   <hr>
   <div id='dataArea'>
      &nbsp;
   </div>
